...
def requests(usuario, contraseña):
    mydb = mysql.connector.connect(
        host = "localhost",
        user = usuario,
        password = contraseña,
    ...
    mycursor = mydb.cursor()
    print("1.- ¿Cuántas noticias fueron publicadas por cada medio de prensa?")
    mycursor.execute("SELECT COUNT(noticia), medio FROM noticia, medio_de_prensa;")

    print("2.- ¿Quienes son las personas mencionadas en las noticias de un día específico?")
    mycursor.execute("SELECT Nombre FROM personas JOIN noticia ON noticia.id_persona=persona.id_persona WHERE fecha_publicaion BETWEEN '2022-07-10' AND '2022-07-17';")

    print("3.- ¿Cómo evoluciona la popularidad de una persona específica?")
    mycursor.execute("SELECT Nombre, Popularidad FROM personas JOIN noticia ON noticia.id_persona=persona.id_persona WHERE Nombre='Gabriel Boric' AND fecha_publicacion BETWEEN '2022-07-10' AND '2022-07-17';")

    print ("4.- ¿Cuáles son los 5 medios de prensa más antiguos en una región especifica?")
    mycursor.execute("SELECT Fecha_Creación from medio_de_prensa ORDER BY ASC LIMIT 5;")
...
def main():
    ...
    requests = input("Quiere hacer las consultas a la base de datos? S/N= ")
    if (requests == "S"):
        requests(usuario,contraseña)
        print("El programa ha finalizado")
    else: 
        print("El programa ha finalizado")
main()  